...
```bash
hugo mod get github.com/c-x-x/hugo-theme-imx@v1.5.0
hugo mod tidy
```
...
首页会先按文章 `date` 倒序整理内容。首屏“最新文章”指向排序后的第一篇；“精选文章”从 `featured = true` 的内容中取最新四篇，没有任何文章显式精选时才回退到最新四篇；下方“最新文章”最多显示最新四篇，不会跳过已经出现在精选区域中的文章，因此两个区域可能出现同一篇文章。精选不足四篇时会占据左侧槽位，从左向右排列。其余内容保留在 `/posts/` 分页列表中，首页不会展开成全部文章归档。

首页字形背景仍以 Canvas 绘制，但会限制高像素密度设备的渲染倍率，以 60 帧为绘制上限，并在页面离开视口时暂停。窗口尺寸变化会延迟合并处理，深浅色切换只更新颜色，不重建整套字形数据。这些调整减少了滚动和切换页面时的持续开销，同时避免用过低帧率换取性能。
...
IMX 的搜索不依赖第三方服务。搜索入口只位于 `/posts/` 文章列表页，不会在文章页、分类页或其他页面显示，也不是全站搜索。进入文章列表页后，浏览器按需加载 Hugo 在首页旁生成的 `index.json`；普通页面访问不会额外请求索引。

索引包含标题、完整纯文本正文、截断摘要、发布日期、分类和标签，最多显示 10 条结果。标题匹配权重最高，标签与分类次之，摘要和正文用于补充；相关度相同时按发布日期倒序。多个空格分隔的关键词采用“全部命中”逻辑。中文单字即可触发搜索，纯英文或数字至少输入两个字符，避免过短查询产生大量无意义结果。

搜索开始后，结果会替换当前分页文章列表，避免同一页面同时出现“搜索结果”和“原列表”两套内容。清空输入、点击清除按钮或按 `Esc` 会恢复原列表。加载中、无结果和索引失败都有独立提示；输入框与结果状态可被读屏软件识别，并针对 320px 等窄屏调整了间距、字号、触控区域和元数据数量。
...
搜索索引只收录 `mainSections` 指定的普通内容页。About 页、分类聚合页和草稿不会混入搜索结果。正文很长或文章很多时，`index.json` 会相应增大；可通过收紧 `mainSections` 控制索引范围，而不必删除 JSON 输出。
...
- `featured`：`true` 时进入首页精选候选；精选区按日期倒序最多显示四篇。
...
本地预览重点看：首页精选与最新文章顺序、首页滚动是否流畅、手机导航、文章列表搜索、长文章目录、代码复制、图片 404、浅色与深色，以及控制台报错。搜索至少检查一个中文单字、一个英文词、多个关键词、清除恢复和 320px 窄屏是否横向溢出。
...
- 首页 JSON 输出存在，文章列表页搜索能按标题、正文、分类和标签返回结果。
- 首页精选与最新文章均按预期显示，首屏链接确实指向日期最新的文章。
