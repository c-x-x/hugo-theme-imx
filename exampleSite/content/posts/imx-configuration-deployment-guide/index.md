...
首页会先按文章 `date` 倒序整理内容。首屏“最新文章”指向排序后的第一篇；“精选文章”从 `featured = true` 的内容中取前三篇，没有任何文章显式精选时才回退到最新三篇；下方“最新文章”固定显示最新三篇，不会跳过已经出现在精选区域中的文章，因此两个区域可能出现同一篇文章。其余内容保留在 `/posts/` 分页列表中，首页不会展开成全部文章归档。
...
- `featured`：`true` 时进入首页精选候选；精选区最多显示三篇。
