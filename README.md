...
IMX 是一个面向中文博客的 Hugo 主题，通过 Hugo Module 安装。它包含文章列表、分类与标签、文章搜索、文章目录、代码复制和 Giscus 评论，适合技术笔记和长期写作。
...
- 浅色、深色和自动三种主题模式
- 首页、文章列表、分类、标签和关于页面
- 文章列表页内置标题、正文、分类和标签搜索
- 首页精选文章与最新文章独立排序
- 首页背景动画按设备像素密度和页面可见状态控制绘制开销
- 文章目录、阅读进度和代码复制
- 可选的 Giscus 评论
...
如果需要固定到某个版本，把 `@latest` 改成对应的 Release 版本号即可，例如 `@v1.5.0`。
...
搜索框只显示在 `/posts/` 文章列表页，不是全站搜索。它依赖首页生成的 `index.json`，请保留 `outputs.home` 中的 `JSON`。

索引只收录 `params.mainSections` 指定的普通内容页，覆盖标题、完整正文、摘要、分类和标签。结果优先匹配标题，其次是标签与分类，最后是摘要和正文；包含多个空格分隔关键词时，每个关键词都必须命中，相关度相同时按文章日期倒序。中文输入一个字符即可搜索，纯英文或数字至少需要两个字符，最多显示十条结果。

输入有效关键词后，搜索结果会临时替换当前分页文章列表；点击清除按钮或按 `Esc` 可恢复列表。输入框、状态提示和结果卡片均已适配键盘操作、读屏状态和窄屏尺寸。
...
`image` 可以省略。将 `toc` 设置为 `false` 可关闭当前文章的目录。将 `featured` 设置为 `true` 可把文章加入首页“精选文章”，主题按 `date` 倒序显示最新的四篇精选文章；没有文章设置该参数时，精选区回退到日期最新的四篇。首页首屏的“最新文章”和下方“最新文章”也以 `date` 为准倒序排列，下方最多显示最新四篇，因此可能与精选文章重合；精选不足四篇时从左向右排列。全部内容仍从“全部文章”或 `/posts/` 进入。

## Mermaid 流程图

在文章中使用 `mermaid` 围栏代码块即可生成流程图：
...
主题通过 Hugo 渲染钩子保留图表源码，并在文章页加载 Mermaid 完成渲染；普通代码块仍使用原有的语法高亮样式。
...
Playwright 会在 `/tmp` 中启动两份独立示例站服务，其中一份启用测试用 Giscus 配置。测试覆盖五种视口、主要页面、搜索相关度与中文单字输入、主题模式、导航、Dock 的合并与恢复、Giscus 主题同步、目录、404 游戏、About 访客完整请求顺序、超时、失败显示与缓存，以及横向溢出。首页、长文章和 About 页分别保留浅色、深色视觉基线；每次测试都会比较差异，并把本次截图保存到 `test-results` 供 CI 下载。
