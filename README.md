...
`image` 可以省略。将 `toc` 设置为 `false` 可关闭当前文章的目录。将 `featured` 设置为 `true` 可把文章加入首页“精选文章”，首页最多显示三篇精选文章；没有文章设置该参数时，主题会回退到日期最新的三篇。首页首屏的“最新文章”和下方“最新文章”都以 `date` 为准倒序排列，下方固定显示最新三篇，因此可能与精选文章重合；全部内容仍从“全部文章”或 `/posts/` 进入。
